pkg<-c('shiny','shinyWidgets','shinythemes','plotly',
       'tidyverse','tidyquant')
...
ui <-fluidPage(#theme =shinytheme('cyborg'),
  ...
  sidebarLayout(
      sidebarPanel(width = 3,
                   ...
                   # Pick benchmark
                   radioGroupButtons("benchmark",label=h4("Benchmark"),
                                     choices = list("SP500"=1,"Nasdaq100"=2,
                                                    "None"=3),
                                     selected = 3)
      ),
      # Plot results
  ...
  observeEvent(c(input$period, input$stocks, input$benchmark),{
    ...
    output$plot<-renderPlotly({
      print(
        ggplotly(prices %>% 
                   group_by(symbol) %>% 
                   mutate(init_close=if_else(date == min(date),
                                             close,NA_real_)) %>% 
                   ...
                   # geom_area(aes(fill=symbol),positon='identity, alpha=.2)
